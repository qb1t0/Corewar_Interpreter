...
# define COMMENT COMMENT_CMD_STRING
...
char *g_er[] = {
    ...
    ": .name filed is absent.\n",
    ": .comment filed is absent.\n",
    ...
    ": NaN(not a number) in operation argument.\n",
...
{
    int             r;      //for register value
    int             cmd;    //код в g_tab 1 byte
    int             start;  //size before reading
    int             line;
    int             size;   //size after reading
    char            cipher; //шифр (rus.) 1 byte
    int             num;
    char            *name;
    char            **args;
